...
class ANN:
    ...
    def compute_error_minibatch(self, batch_desired_pattern, batch_predicted_pattern):
         ...
         error = sum( i*i for i in self.error_at_output_layer)
    ...
    def compute_error_vector(self, test_data):
        ...
        for x,y in test_data:
            ...
            error = error + self.compute_error_minibatch(prediction, x)
            #print error
        #print error
    ...
    def compute_error(self, desired_pattern, predicted_pattern):
         ...
         error = error + np.sqrt(np.mean(np.square(self.error_at_output_layer))) 
         MSE = float(error / len(desired_pattern))
         #print MSE
         return MSE
    ...
    def backprop(self, x, y):
        ...
        delta = self.compute_error(a, y) * self.activation(zs[-1],count-1,1) 
        nabla_b[-1] = delta
